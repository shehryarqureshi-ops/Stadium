...
export type SwagPlatformContent = {
  eyebrow: string;
  heading: string;
  body: string;
  cards: { img?: StaticImageData; title: string; desc: string }[];
};

export const SWAG_PLATFORM: SwagPlatformContent = {
  eyebrow: "THE PLATFORM",
  heading: "The stack behind every send",
  body: "Everything you need to run swag at scale, from storefronts and inventory to automation and reporting.",
  cards: [
...
export default function SwagPlatform({ content = SWAG_PLATFORM }: { content?: SwagPlatformContent }) {
  return (
        ...
        <div data-animation="reveal" className="w-full rounded-[2.5rem] bg-[#f7f7f7] p-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {content.cards.map((c) => (
              ...
              >
                <div className="relative flex aspect-[376/260] w-full items-center justify-center overflow-hidden rounded-lg bg-grey-200">
                  {c.img ? (
                    <Image
                      src={c.img}
                      alt=""
                    ...
                    <svg className="size-9 text-grey-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" aria-hidden>
