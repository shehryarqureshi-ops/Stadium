    ...
    <section className="bg-white px-section-x-sm py-20 md:px-section-x-md md:py-24 lg:px-section-x-lg lg:py-28">
      <div className="mx-auto flex w-full max-w-content flex-col gap-10">
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <p data-animation="reveal" className="max-w-[46rem] font-sans text-body-md text-swag-grey">
        ...
        <div data-animation="reveal" className="flex flex-col gap-8">
          <div ref={track} onScroll={sync} className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {QUOTES.map((q, i) => (
              <article key={i} className="relative flex h-[19.5rem] w-[85vw] shrink-0 snap-start rounded-2xl bg-[#f2f2f2] p-4 sm:w-[28rem]">
                <div className="absolute left-4 top-4 z-10 size-14 overflow-hidden rounded-lg">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src="/swag/swag-ribbon.jpg" alt="" aria-hidden className="size-full object-cover" />
                </div>
                <div className="ml-[4.5rem] flex flex-1 flex-col justify-between rounded-xl bg-white p-7">
                  <span className="w-fit rounded-full bg-[#e8f8ee] px-3 py-1 font-sans text-[0.75rem] font-bold text-swag-green-deep">
                    {q.stat}
                  </span>
                  <div className="flex flex-col gap-4">
                    <p className="font-[family-name:var(--font-satoshi-medium)] text-[1.375rem] leading-[1.2] text-swag-ink">
                      &ldquo;{q.quote}&rdquo;
                    </p>
