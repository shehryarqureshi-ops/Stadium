...
function Value({ v }: { v: Cell }) {
  if (v === "check")
    return (
      <svg viewBox="0 0 24 24" fill="none" stroke="#16171b" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="size-6" aria-label="Yes">
        <path d="M20 6 9 17l-5-5" />
      </svg>
    );
  if (v === "minus")
    return (
      <svg viewBox="0 0 24 24" fill="none" stroke="#b4b4b8" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="size-6" aria-label="No">
        <path d="M5 12h14" />
      </svg>
    );
  if (v.startsWith("check:"))
    return (
      <span className="flex items-center gap-1.5">
        <svg viewBox="0 0 24 24" fill="none" stroke="#16171b" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="size-6" aria-hidden>
          <path d="M20 6 9 17l-5-5" />
        </svg>
        <span className="font-sans text-[0.90625rem] font-semibold leading-[1.4] text-[#16171b]">{v.slice(6)}</span>
      </span>
    );
  return <span className="font-sans text-[0.90625rem] font-semibold leading-[1.4] text-[#16171b]">{v}</span>;
}
        ...
        <div data-animation="reveal" className="w-full overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <div className="grid min-w-[52rem] grid-cols-[2.09fr_1fr_1fr_1fr_1fr_1fr] gap-1">
            ...
            {COLS.map((c) => (
              ...
              >
                <span className={`font-sans text-[0.90625rem] leading-[1.4] text-[#16171b] ${c.hi ? "font-semibold" : ""}`}>{c.name}</span>
                <span className="font-sans text-[0.6875rem] leading-[1.4] text-[#6b6c71]">{c.sub}</span>
              </div>
            ))}

            {ROWS.map((r, ri) => (
              <div key={r.label} className="contents">
                <div className={`flex items-center rounded-[0.5rem] bg-[#f2f2f2] p-6 ${ri === 0 ? "rounded-tl-[1.5rem]" : ""} ${ri === ROWS.length - 1 ? "rounded-bl-[1.5rem]" : ""}`}>
                  <span className="font-sans text-[0.90625rem] font-semibold leading-[1.4] text-[#16171b]">{r.label}</span>
                </div>
                {r.vals.map((v, i) => (
                  <div
                    key={i}
