...
export type SwagAdminViewContent = {
  eyebrow: string;
  heading: string;
  body: string;
  image: {
    /** next/image src: a static import (StaticImageData) or a string path like "/swag/admin-view.png". */
    src: StaticImageData | string;
    alt: string;
  };
};

export const SWAG_ADMINVIEW: SwagAdminViewContent = {
  eyebrow: "THE ADMIN VIEW",
  // ’ = right single quotation mark (renders identically to &rsquo;)
  heading: "One view of every team’s store",
  body: "Stores by team, live order counts, and per-team budgets, backed by a permission model Procurement signs off on.",
  image: {
    src: adminView,
...
export default function SwagAdminView({
  content = SWAG_ADMINVIEW,
}: {
  content?: SwagAdminViewContent;
}) {
  return (
    <section className="bg-white px-section-x-sm pb-16 pt-20 md:px-section-x-md md:pb-20 md:pt-24 lg:px-section-x-lg lg:pb-24 lg:pt-28">
      <div className="mx-auto flex w-full max-w-content flex-col items-center gap-10">
        ...
        {/* real Figma dashboard mockup — static imports use next/image
            (dimensions inferred); string paths fall back to a plain <img>
            since next/image would require explicit width/height. */}
        <div
          data-animation="reveal"
